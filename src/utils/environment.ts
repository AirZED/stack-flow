...
/** In dev uses Vite proxy (/api/stacks); in production uses Hiro API directly. */
export function getStacksApiBase(): string {
  if (import.meta.env.DEV) return '/api/stacks';
  return (import.meta.env.VITE_STACKS_API_URL || 'https://api.mainnet.hiro.so').replace(/\/$/, '');
}
